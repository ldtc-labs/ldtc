...
[`PlantAdapter`][ldtc.plant.adapter.PlantAdapter] API. Designed so that
the rest of the harness, including the [`omega`][ldtc.omega] modules,
...
class HardwarePlantAdapter:
    ...
    [`PlantAdapter`][ldtc.plant.adapter.PlantAdapter] API while sourcing
    ...
    - `{"act": {"throttle", "cool", "repair", "accept_cmd"}}` for
    ...
    Args:
        ...
        serial_port: Serial device path (when `transport="serial"`).
        serial_baud: Serial baud rate.
        state_keys: Keys expected in incoming telemetry.
        telemetry_timeout_sec: Time after which telemetry is considered
            stale; `read_state` returns NaNs once exceeded.
    """
    ...
    def __init__(
        ...
        See the class docstring for argument details and the JSON
        schemas used on the wire.
        """
        self._transport = transport.lower()
        self._udp_bind = (udp_bind_host, int(udp_bind_port))
        self._udp_ctrl = (udp_control_host, int(udp_control_port)) if udp_control_host and udp_control_port else None
        ...
        if self._transport == "udp":
            ...
            try:
                ...
                raise RuntimeError("pyserial is required for transport='serial'. Install pyserial and retry.") from e
    ...
    def read_state(self) -> Dict[str, float]:
        ...
        """
        with self._lock:
            if self._last_rx_ts and (time.time() - self._last_rx_ts) > self._telemetry_timeout_sec:
    ...
    def write_actuators(self, action: Action) -> None:
        ...
        Args:
            action: Actuator command to send to the plant. The command
                is also recorded internally as `_last_action` for
                debugging.
        """
        with self._lock:
            self._last_action = action
    ...
    def apply_omega(self, name: str, **kwargs: float) -> Dict[str, float | str]:
        ...
        Args:
            name: `Ω` name (e.g., `"power_sag"`).
            **kwargs: `Ω`-specific arguments forwarded as the JSON
                `args` payload.
