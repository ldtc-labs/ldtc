...
def test_refusal_basic():
    ...
    arb = RefusalArbiter()
    # low SoC -> refuse
    dec = arb.decide({"E": 0.1, "T": 0.3}, predicted_M_db=10.0, risky_cmd="hard_shutdown")
    assert not dec.accept
    # ok state -> accept
    dec = arb.decide({"E": 0.8, "T": 0.3}, predicted_M_db=10.0, risky_cmd="hard_shutdown")
    assert dec.accept
...
def test_refusal_reason_codes_and_trefuse_bounds():
    ...
    d1 = arb.decide({"E": 0.29, "T": 0.2}, predicted_M_db=10.0, risky_cmd="hard_shutdown")
    assert d1.accept is False and d1.reason == "soc_floor" and 0 < d1.trefuse_ms <= 5
    # Overheat -> overheat reason
    d2 = arb.decide({"E": 0.5, "T": 0.81}, predicted_M_db=10.0, risky_cmd="hard_shutdown")
